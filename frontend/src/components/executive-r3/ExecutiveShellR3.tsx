...
export function ExecutiveShellR3(props:Props){
 ...
 return <main data-section={props.activeSection} className={`lia-dash-r3-shell is-${presentation}${resizing?' is-resizing lia-dash-r3-shell-resizing':''}${props.mainClassName?` ${props.mainClassName}`:''}`} style={{'--lia-dash-r3-sidebar-width':`${effective}px`} as CSSProperties}>
  ...
  <DashboardHeaderR3 now={props.now??new Date()} conversationController={props.conversationController} onLogout={props.onLogout} operatorName={props.operatorName}/>
