...
function parseProjectTaskSqlitePath(rawPath: string | undefined): string {
  const candidate = rawPath?.trim() ?? '';
  ...
  if (!isAbsolute(candidate) || candidate.includes('\0')) {
    throw new Error('invalid_lia_project_task_sqlite_path');
  }
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): LiaAgentConfig {
  ...
  return {
    ...
    projectTaskSqlitePath: parseProjectTaskSqlitePath(env.LIA_PROJECT_TASK_SQLITE_PATH),
