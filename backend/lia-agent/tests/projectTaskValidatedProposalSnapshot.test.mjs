...
test('3. V4 and V9 migration chains reach V13 additively with zero manufactured snapshots', async () => {
  for (const [label, rewindSql, expectedVersion, dynamicPlansDrop] of [
    ...
    try {
      ...
      assert.equal(check.prepare('SELECT schema_version FROM project_task_meta').get().schema_version, 19, label);
...
// ===========================================================================
test('30. migration chain asserts SCHEMA_VERSION 19 in every affected test file', async () => {
  assert.equal(PROJECT_TASK_SQLITE_SCHEMA_VERSION, 19);
  for (const file of [
    ...
    assert.equal(source.includes('PROJECT_TASK_SQLITE_SCHEMA_VERSION, 19'), true, file);
